    ...
    <script>
        ...
        function listMedia() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    response[0].forEach(function(key){
                        $('#gallery').append('<button type="button" class="btn lazy gallery-btn m-1 p-1" onclick="galleryBtn('+key.id+', `'+key.name+'`)" value="'+key.id+'"><img class="gallery-image lazy" src="{{ asset() }}'+key.path+'"></button>');
                    })
                }
            });
        }
